...
def append_value(output, value, position, force_half=False):
    ...
    if value > 0xFF or force_half:
        assert output[position-offset] is None and output[position-offset+1] is None

        output[position-offset] = value & 0xFF
        output[position-offset+1] = (value >> 8) & 0xFF
        return position + 2
    else:
        assert output[position-offset] is None
...
def append_placeholder_label(output, label, position):
    ...
    output[position-offset] = 0x00
    output[position-offset+1] = 0x00
    return position + 2
...
if __name__ == "__main__":
    ...
    output_bytes = [None for i in range(ROM_SIZE)]  # Output bytes for bin file
    seekstack = [] # stack of pushed seeks
    ...
    while token_pos < len(tokens):
        ...
        if token.startswith("/*"):
            commenting = True  # Start comment

        if commenting:
            if token.endswith("*/"):
                commenting = False  # End comment
            token_pos += 1
        ...
        if instruction_from_name(token) is not None:  # Instruction
            ...
            if cmd == "offset":
                ...
                token_pos += 1
                value = int(tokens[token_pos], 0) & 0xFFFF
                seekstack.append(position)
                position = value

            elif cmd == "seekpop":
                # Seek to a specific address, fill bytes before then with zeros
                position = seekstack.pop(-1)
    ...
    try:
        with open(output_file, "wb") as output_stream:
            output_stream.write(bytes([0x00 if b is None else b for b in output_bytes]))
    except:
