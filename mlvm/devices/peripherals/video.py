from mlvm.devices import Peripheral
from mlvm.const import *

import os

os.environ["PYGAME_HIDE_SUPPORT_PROMPT"] = "1"

import pygame as pg
import colorsys

VIDEO_WIDTH = 0x100
VIDEO_HEIGHT = 0x100

REG_X = 0x00
REG_Y = 0x01
REG_W = 0x02
REG_H = 0x03

REG_PIXEL = 0x04
REG_FILL = 0x05
REG_RECT = 0x06

EVENT_CYCLE_INTERVAL = 1000

DISPLAY_SCALE = 4

FLIP_NMI_ID = 0x40


def byte_to_rgb(color_byte):
    """
    Convert a pseudo-HSV byte to RGB
    """

    hv = color_byte & 0b11111000

    if hv == 0b00111000:
        return (255, 255, 255)

    if hv == 0:
        return (0, 0, 0)

    hn = color_byte & 0b00000111
    vn = (color_byte >> 3) & 0b00000111
    sn = (color_byte >> 6) & 0b00000011

    h = hn / 8
    v = (vn + 1) / 8
    s = (sn + 1) / 4

    r, g, b = colorsys.hsv_to_rgb(h, s, v)

    return (r * 255, g * 255, b * 255)


class MLVMVideoInterface(Peripheral):
    """
    Video peripheral

    Opens up a display that can be drawn to and updated
    """

    def __init__(self, bus, peripheral_id):
        super().__init__(bus, peripheral_id)
        self.video_ram = [0x00 for i in range(VIDEO_WIDTH * VIDEO_HEIGHT)]

        self.reg_x = 0
        self.reg_y = 0
        self.reg_w = 0
        self.reg_h = 0

        self.screen = pg.display.set_mode(
            (VIDEO_WIDTH * DISPLAY_SCALE, VIDEO_HEIGHT * DISPLAY_SCALE),
            pg.DOUBLEBUF | pg.HWACCEL | pg.HWSURFACE,
        )
        pg.display.set_caption("MLVM")
        pg.display.set_icon(pg.Surface((1, 1)))

        self.surface = pg.Surface((VIDEO_WIDTH, VIDEO_HEIGHT))
        self.clock = pg.time.Clock()

    def set_pixel(self, x: int, y: int, c: int):
        self.video_ram[y * VIDEO_WIDTH + x] = c
        self.surface.set_at((x, y), byte_to_rgb(c))

    def get_pixel(self, x: int, y: int):
        return self.video_ram[y * VIDEO_WIDTH + x]

    def fill(self, c: int):
        for i in range(len(self.video_ram)):
            self.video_ram[i] = c

        self.surface.fill(byte_to_rgb(c))

    def clock_neg(self):
        if self.bus.address in self.addr_range and self.bus.intent == WRITE:
            reg = self.unoffset_addr(self.bus.address)

            if reg == REG_X:  # Set X position
                self.reg_x = self.bus.data % VIDEO_WIDTH

            elif reg == REG_Y:  # Set Y position
                self.reg_y = self.bus.data % VIDEO_HEIGHT

            if reg == REG_W:  # Set X position
                self.reg_w = self.bus.data % VIDEO_WIDTH

            elif reg == REG_H:  # Set Y position
                self.reg_h = self.bus.data % VIDEO_HEIGHT

            elif reg == REG_PIXEL:  # Write pixel of this color at X and Y
                self.set_pixel(self.reg_x, self.reg_y, self.bus.data)

            elif reg == REG_FILL:  # Fill the screen with this color
                self.fill(self.bus.data)

            elif reg == REG_RECT:
                for x in range(self.reg_x, self.reg_x + self.reg_w):
                    for y in range(self.reg_y, self.reg_y + self.reg_h):
                        self.set_pixel(x, y, self.bus.data)

        if self.bus.cycle % (CPU_GOAL_CLOCK // 60) == 0:
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    exit()

            self.bus.nmi(FLIP_NMI_ID)

            pg.transform.scale_by(self.surface, DISPLAY_SCALE, self.screen)
            pg.display.flip()
            self.clock.tick(-1)
            pg.display.set_caption(f"MLVM FPS: {int(self.clock.get_fps())}")
