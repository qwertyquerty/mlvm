...
class CompilerStateMachine:
    ...
    def __init__(self, mlvc_filename, cwd):
        ...
        self.asm_block_in_directive = False
        self.asm_directive_tokens = []
    ...
    def include_file(self, filename):
        ...
        self.cur_line = 1
        self.cur_file = os.path.join(self.cwd, f"{filename}")
    ...
    def process_keyword(self, token):
        ...
        elif token == "rti":  # Return from an interrupt
    ...
    def process(self, token):
        ...
        if (
        ...
        ):  # While were in a comment just wait for the close comment operator, otherwise do nothing
            if token == "*/":
            ...
            token in self.defines
        ):  # We found a preprocessor define so just process the tokens of the value of that define inline here
            ...
            self.state == self.STATE_ASM and self.asm_block_in_directive and token == "\n"
        ):  # A newline terminates a directive's arguments inside an asm block
            self.asm += f"{' '.join(self.asm_directive_tokens)}\n"
            self.asm_block_in_directive = False
            ...
            if token == ";":
                path = os.path.join(*self.path_builder)
                self.path_builder = []
                self.state_stack.pop(-1)
                ...
                self.path_builder.append(token)

        elif (
            self.state == self.STATE_VAR
        ):  # We are defining variables, allocate each token as a variable symbol until semicolon
            ...
            if self.function_builder_name is None:  # We haven't defined the name yet so wait for that
                ...
                if token != "{":
                    ...
                    self.state_stack.append(
                        self.STATE_BRACE_CONTENT
                    )  # We found an opening brace so move to the brace content state
            ...
            if self.asm_block_brace_state:  # We are in the content of the assembly block
                if token == "{":
                    ...
                    if self.asm_block_in_directive:
                        self.syntax_error("Unterminated directive in asm block!")

                    self.asm += "    /* End MLVC asm block */\n"
                    self.asm_block_brace_state = False
                    self.state_stack.pop(-1)
                elif self.asm_block_in_directive:  # We are in the arguments of a directive, collect tokens raw until a newline
                    self.asm_directive_tokens.append(token)
                else:
                    if (
                        self.asm_block_element_type is not None
                    ):  # We saw an operator last token that hints at the type of the next token
                        if (
                            self.asm_block_element_type == self.ASM_ELEMENT_TYPE_INSTRUCTION
                        ):  # Process token as an instruction
                            ...
                            if token in self.static_vars:
                                ...
                                self.append_instruction(f"${self.asm_prefix_function(token)}")
                            else:
                        ...
                        if token == "#":  # Instruction indicator
                            self.asm_block_element_type = (
                                self.ASM_ELEMENT_TYPE_INSTRUCTION
                            )  # Next token should be an instruction
                        elif token == "@":  # Pointer indicator
                            self.asm_block_element_type = self.ASM_ELEMENT_TYPE_POINTER  # Next token should be a symbol
                        elif token.startswith("."):  # Directive indicator, args accumulate on the same output line until a newline
                            self.asm_directive_tokens = [token]
                            self.asm_block_in_directive = True
                        elif re.fullmatch(SYMBOL_RE + ":", token):  # Label, pass straight through to the assembler
                            self.asm += f"{token}\n"
            ...
            self.state == self.STATE_SET_RHS
        ):  # We are in the right hand side of a set block, start building expression
            ...
            if token == "{":  # Opening brace so we are in the content
                ...
                self.asm += (
                    f"loop_{self.cur_loop_id}_begin: /* Begin MLVC while block */\n"  # Opening label to jump to at end
                )
                self.asm += "    /* MLVC if expression */\n"
            ...
            if token in KEYWORDS:  # We found a keyword so process it to set up the state for it
                ...
                if token == "{":  # Opening brace so append new brace content to state stack
                    ...
                    token == "}"
                ):  # We found a closing brace, so we need to figure out what the previous keyword was before the open brace and clean up
                    self.state_stack.pop(-1)

                    if (
                        self.state_stack[-1] == self.STATE_FUNCTION_NAME
                    ):  # We are closing a function so add a return always
                        self.state_stack.pop(-1)
                        self.function_builder_name = None
                        self.append_instruction("RET")
                        self.asm += "\n"
                        ...
                        self.state_stack[-1] == self.STATE_IF
                    ):  # We are closing an if statement so add the else label to jump do if the conditional is false
                        self.state_stack.pop(-1)
                        conditional_id = self.conditional_stack.pop(-1)
                        self.asm += f"conditional_{conditional_id}_else: /* End MLVC if statement */\n"

                    elif (
                        self.state_stack[-1] == self.STATE_LOOP
                    ):  # We are closing a loop statement so add a jump to the beginning and a label for jumping out of the loop
                        self.state_stack.pop(-1)
        ...
        for i, element in enumerate(rpn):
            if element in EXPRESSION_OPERATOR_PRECEDENCE:
                ...
                if a is not None and b is not None and re.match(VALUE_RE, a) and re.match(VALUE_RE, b):
                    ...
                    c = self.evalute_const_operation(
                        int(a, 0),
                        int(b, 0),
